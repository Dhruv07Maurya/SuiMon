...
function AppContent() {
  ...
  useEffect(() => {
    // Ping the backend once when the component mounts
    fetch("https://suimon.onrender.com/")
      .then((response) => {
        if (response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 transition-all duration-300 ease-in-out">
      ...
      <header className="bg-gray-800/80 backdrop-blur-sm border-b border-gray-700 p-6 shadow-lg">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-2">
            <div
              className={`${bgColors.primary} w-3 h-8 rounded-full ${glowingEffect}`}
            ></div>
            <div
              className={`${bgColors.secondary} w-3 h-8 rounded-full ${glowingEffect}`}
            ></div>
            <div
              className={`${bgColors.accent1} w-3 h-8 rounded-full ${glowingEffect}`}
            ></div>
            <h1
          ...
          <div className="flex space-x-4 items-center">
            <div className="hidden md:block">
              <div className="flex space-x-2">
                <div
                  className={`h-3 w-3 rounded-full ${bgColors.primary} animate-pulse`}
                ></div>
                <div
                  className={`h-3 w-3 rounded-full ${bgColors.secondary} animate-pulse delay-100`}
                ></div>
                <div
                  className={`h-3 w-3 rounded-full ${bgColors.accent1} animate-pulse delay-200`}
                ></div>
              </div>
            </div>
            {isAuthenticated ? (
          ...
          <div className="container mx-auto flex flex-wrap justify-center md:justify-between gap-4">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-md ${getNeonBorder(
                  tab.id
                )} ${getTabColor(
                  tab.id
      ...
      <main
        className={`container mx-auto p-6 transition-all duration-500 ${
